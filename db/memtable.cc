...
bool MemTable::CheckAdaptive(){
  return table_ ->isAdaptive();
}

std::vector<Slice> MemTable::GetEndpointList(){
  return table_->GetEndpointList();
}

void MemTable::RebuildEndpointList(std::vector<Slice> endpoints){
  table_->RebuildEndpointList(endpoints);
}
