...
struct FileMetaData {
  ...
  //if use adaptive linklist, the endpoint list
  std::vector<Slice> endpoint_list;
  ...
  //if use adaptive linklist, need to update endpoint_list, otherwise the 
...
class VersionEdit {
 public:
  ...
  void AddFile(int level, uint64_t file, uint32_t file_path_id,
    ...
    assert(smallest_seqno <= largest_seqno);
    FileMetaData meta_data(file, file_path_id, file_size, smallest, largest,
                            smallest_seqno, largest_seqno,
                            marked_for_compaction, oldest_blob_file_number,
                            oldest_ancester_time, file_creation_time,
                            file_checksum, file_checksum_func_name);
    if(endpoint_list_.size() != 0)
      meta_data.UpdateEndpointList(endpoint_list_);
    new_files_.emplace_back(
        level, meta_data);
  }
