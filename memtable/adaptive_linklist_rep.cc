...
typedef SkipList<char*, const MemTableRep::KeyComparator&> MemtableSkipList;
...
struct BucketHeader {
  ...
  Key GetEndPointKey() const {
    return endpoint_key.load(std::memory_order_relaxed);
  }
...
class AdaptiveLinkListRep : public MemTableRep {
 public:
  AdaptiveLinkListRep(const MemTableRep::KeyComparator& compare,
                  ...
                  MemtableSkipList endpoint_list,
                  int endpoint_num,
 ...
 private:
  ...
  //SkipListBucketHeader* GetSkipListBucketHeader(Pointer* first_next_pointer) const;
...
/*
  ...
  */
  ...
  size_t FindRange(const Slice& slice) const {
    size_t low = 0;
    size_t high = endpoint_num_ - 1;
    size_t middle ;
    while (low < high){
      middle = (low + high)/2;
      Pointer* header =  buckets_[middle].load(std::memory_order_acquire);
      Key endpoint = reinterpret_cast<BucketHeader*>(header).GetEndPointKey();
      if(endpoint == nullptr) return middle;
      if (compare_(slice, endpoint) == 0) return middle;
      else if (compare_(slice, endpoint) < 0){
          Key prev_endpoint = reinterpret_cast<BucketHeader*>(buckets_[middle - 1].load(std::memory_order_acquire)).GetEndPointKey();
        if (compare_(slice, prev_endpoint) > 0)  return middle;
        else  high = middle - 1;
      }else
        low = middle + 1;
    }
  ...
  //no order in the linkedlist, so not supported
  /*
  ...
  class LinkListIterator : public MemTableRep::Iterator {
   public:
    ...
    void Seek(const Slice& internal_key,
              const char* /*memtable_key*/) override {
      node_ = hash_link_list_rep_->FindWetherInBucket(head_,internal_key);
    }
...
AdaptiveLinkListRep::AdaptiveLinkListRep(
    ...
    MemtableSkipList endpoint_list,int endpoint_num,
    //size_t bucket_size,uint32_t threshold_use_skiplist, 
    size_t huge_page_tlb_size, Logger* logger,
  ...
  char* mem = allocator_->AllocateAligned(sizeof(Pointer) * endpoint_num_,
  ...
  if (endpoint_num_ != 0){
    for (size_t i = 0; i < endpoint_num_; ++i) {
      ...
      Key k = static_cast<Key> (iter.key());    
      header = new (mem2) BucketHeader(first, 0,k);
  ...
  BucketHeader* header = nullptr;
  auto* mem2 = allocator_->AllocateAligned(sizeof(BucketHeader));
  header = new (mem2) BucketHeader(first, 0,nullptr);
...
KeyHandle AdaptiveLinkListRep::Allocate(const size_t len, char** buf) {
...
Node* AdaptiveLinkListRep::GetLinkListFirstNode(Pointer* first_next_pointer) const {
  /* only one case no need to check
  if (first_next_pointer == nullptr) {
    return nullptr;
  }
  if (first_next_pointer->load(std::memory_order_relaxed) == nullptr) {
...
void AdaptiveLinkListRep::Insert(KeyHandle handle) {
  ...
  if (bucket_entries_logging_threshold_ > 0 &&
    ...
    Info(logger_, "AdaptiveLinkedList bucket %" ROCKSDB_PRIszt
                  " has more than %d "
...
bool AdaptiveLinkListRep::Contains(const char* key) const {
  ...
  BucketHeader* header = GetLinkListFirstNode(bucket)；
...
MemTableRep* AdaptiveLinkListRepFactory::CreateMemTableRep(
  ...
  return new AdaptiveLinkListRep(compare, allocator, transform, endpoint_list_,
                             endpoint_num_, huge_page_tlb_size_,
                             logger, bucket_entries_logging_threshold_,
                             if_log_bucket_dist_when_flash_);
}

MemTableRepFactory* NewAdaptiveLinkListRepFactory(
    SkipList<Key, const MemTableRep::KeyComparator&> endpoint_list,
    int endpoint_num,
    size_t huge_page_tlb_size,
    int bucket_entries_logging_threshold, bool if_log_bucket_dist_when_flash) {
  return new AdaptiveLinkListRepFactory(endpoint_list,endpoint_num,huge_page_tlb_size,
