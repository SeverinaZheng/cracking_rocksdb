...
class MemTableRep {
 public:
  ...
  //check if is adaptive linklist
  virtual bool isAdaptive() {return false;}
  virtual std::vector<Slice> GetEndpointList() {return std::vector<Slice>();}
  virtual void RebuildEndpointList(std::vector<Slice> /*endpoints*/) {};
...
extern MemTableRepFactory* NewHybridLinkListRepFactory(
    size_t huge_page_tlb_size = 0,
    int bucket_entries_logging_threshold = 4096, 
    bool if_log_bucket_dist_when_flash = false,
    uint32_t threshold_use_skiplist = 256);


extern MemTableRepFactory* NewAdaptiveLinkListRepFactory(
    size_t huge_page_tlb_size = 0, 
    int bucket_entries_logging_threshold = 4096, 
    bool if_log_bucket_dist_when_flash =false);
#endif  // ROCKSDB_LITE
